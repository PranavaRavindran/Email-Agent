...
root_agent = Agent(
    ...
    instruction=(
        ...
        "explicit staged diff with has_changes false. Re-request staging ONLY when "
        "tracker_agent's response contains no staged counts whatsoever — not one "
        "of new rows, status changes, unchanged count, or duplicates is mentioned. "
        "A response that reports any of those counts is a completed staging run, "
        "even when every count is zero, and must never be re-run. When "
        "re-requesting, state what you observed, not what you inferred: say that "
        "tracker_agent's response did not include staged counts, and that you are "
        "asking it to stage again. Never assert that staging failed to happen — "
        "you cannot know that from a prose summary, and stating it as fact is not "
        "something the response gives you evidence for.\n"
